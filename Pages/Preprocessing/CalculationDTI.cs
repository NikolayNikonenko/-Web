...
namespace перенос_бд_на_Web.Pages.Preprocessing
{
    public class CalculationDTI
    {
        ...
        {
            string dirName = "D:\\учеба\\магистратура\\диплом\\правильные срезы\\100 СРЕЗОВ АВТОФИЛЬТР";
            ...
            foreach (DirectoryInfo dir in dirs)
            {
                ...
                //"roc_debug_after_OC*"
                foreach (FileInfo pathFile in dir.GetFiles())
                {
                    ...
                    //List<double> containsException = new List<double>()

                    //{
                    //    78, 2011, 2807, 2810,4032,4038, 4057, 4103, 4104, 4105, 4106, 4109, 4110, 6103, 6273, 6279, 8003
                    //};
                    ...
                    for (int q = 0; q < numbersNag.Count; q++)
                    {
                        ...
                        for (int b = 0; b < nodesNag.Count; b++)
                        {
                            ...
                            CalculatePTI(Rastr, nodesNag[b], nodeNamesNag[q], ref _nachZnachIdentificator);
                    ...
                    for (int s = 0; s < numbersGen.Count; s++)
                    {
                        ...
                        for (int b = 0; b < nodesGen.Count; b++)
                        {
                            ...
                            CalculatePTI(Rastr, nodesGen[b], nodeNamesGen[s], ref _nachZnachIdentificator);


                        }
                        Console.WriteLine($"Проведен расчет ПТИ нагрузки для ТМ № {numbersGen[s]}");
                    }


                    Match match = regex.Match(Convert.ToString(dir));
                    string SaveFile = @"D:\учеба\магистратура\диплом\правильные срезы\тест ПТИ\тестп ПТИ с расчетом потерь";
                    string subpath = string.Concat(match.ToString(), " после учета потерь");
        ...
        public static void CalculatePTI(IRastr Rastr, double nodesNag, string nodeNamesNag, ref int _nachZnachIdentificator)
        {
            // Обращение к таблице ветви
            ITable _vetvi = (ITable)Rastr.Tables.Item("vetv");
            // Обращение к колонке Nнач
